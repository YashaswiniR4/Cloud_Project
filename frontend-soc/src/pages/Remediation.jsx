...
import { Shield, RefreshCw, Lock, Zap, CheckCircle, Trash2 } from 'lucide-react';
import { getRemediations, clearRemediations } from '../services/api';
...
export const Remediation = () => {
  ...
  const handleClearRemediations = async () => {
    try {
      setClearing(true);
      await clearRemediations();
      setRemediations([]);
    } catch (err) {
      console.error('Failed to clear remediations:', err);
    } finally {
      setClearing(false);
    }
  };
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-2">
          <button
            onClick={handleClearRemediations}
            disabled={clearing || remediations.length === 0}
            className="flex items-center space-x-1.5 px-3.5 py-2 bg-slate-900 hover:bg-slate-800 disabled:opacity-50 text-slate-300 font-medium rounded-xl text-xs border border-slate-800 transition-colors"
            title="Clear duplicate test logs"
          >
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="glass-panel p-4 rounded-xl border border-slate-800 space-y-1">
              <span className="text-xs font-semibold text-slate-400">Unique Targets Contained</span>
              <p className="text-2xl font-bold text-red-400">{remediations.length}</p>
              <p className="text-[10px] text-slate-500">Active Blocked IP Subnets & IAM Users</p>
            </div>
            <div className="glass-panel p-4 rounded-xl border border-slate-800 space-y-1">
          ...
          <div className="glass-panel p-5 rounded-2xl border border-slate-800 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h3 className="text-sm font-bold text-white flex items-center space-x-2">
                ...
                <span>Active Serverless Containment Vault ({remediations.length} Unique Targets)</span>
              </h3>
            </div>
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-red-400 flex items-center space-x-1.5">
                        ...
                        {rem.execution_count > 1 && (
                          <span className="px-2 py-0.5 bg-red-950 text-red-300 border border-red-800/60 rounded-full text-[10px] font-mono">
                            {rem.execution_count}x Executed
                          </span>
                        )}
                      </span>
                      <span className="text-xs font-mono text-blue-400 font-bold">Target: {rem.target_identifier || 'Attacker IP'}</span>
                    </div>

                    <div className="space-y-1.5 pt-1">
                      {rem.actions_taken?.map((action, aIdx) => (
                        <div key={aIdx} className="text-xs font-mono text-slate-200 bg-slate-900/80 p-2.5 rounded-lg border border-slate-800 flex items-center justify-between">
                          <span className="flex items-center space-x-2">
                            <CheckCircle className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                            <span>{action}</span>
                          </span>
                          <span className="text-[10px] text-slate-500 font-sans">Security Group sg-0a1b2c3d4e5f6789a</span>
                        </div>
                      ))}
                    </div>
                  </div>
